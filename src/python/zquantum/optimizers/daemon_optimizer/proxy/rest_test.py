...
class TestOptimizationServer(unittest.TestCase):
    ...
    def test_ping_204(self):
        connection = http.client.HTTPConnection(
            self.ipaddress + ":" + str(self.listening_port), timeout=2
        )
    ...
    def test_post_current_argument_values(self):
        ...
        save_circuit_template_params(
            params, "proxy_test_current_argument_values_artifact.json"
        )
        ...
        with open(
            "proxy_test_current_argument_values_artifact_from_proxy.json", "w"
        ) as f:
            f.write(json.dumps(response_json))
        new_data_loaded_from_file = load_circuit_template_params(
    ...
    def test_unsuccessful_post_argument_values_invalid_JSON(self):
        ...
        connection.request(
            "POST", "/cost-function-argument-values", body="invalid JSON"
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 400)
        response_lower = response.read().decode("utf-8").lower()
    ...
    def test_unsuccessful_post_argument_values_no_keys_JSON(self):
        ...
        connection.request(
            "POST",
            "/cost-function-argument-values",
            body=json.JSONEncoder().encode("invalid JSON"),
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 400)
        response_lower = response.read().decode("utf-8").lower()
    ...
    def test_post_result(self):
        ...
        connection.request(
            "POST",
            "/cost-function-argument-values",
            body=json.JSONEncoder().encode(arg_val_data),
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 200)
        # decode id from response
        ...
        connection.request(
            ...
            body=json.JSONEncoder().encode(result_data),
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 204)
        ...
        connection.request(
            "GET", "/cost-function-results", body=id_from_argument_value_post
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 200)
        ...
        with open("proxy_test_results_artifact_from_proxy.json", "w") as f:
            f.write(json.dumps(response_json))
        new_data_loaded_from_file = load_value_estimate(
    ...
    def test_unsuccessful_get_result_no_id(self):
        ...
        # GET cost function result
        connection.request(
            "GET", "/cost-function-results"
        )  # Will fail bc there's no ID in the body
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 400)
        response_lower = response.read().decode("utf-8").lower()
        self.assertTrue(response_lower.find("error") != -1)
    ...
    def test_unsuccessful_post_result_no_keys_JSON(self):
        ...
        connection.request(
            "POST",
            "/cost-function-results",
            body=json.JSONEncoder().encode("invalidJSON"),
        )
        response = connection.getresponse()
        self.assertEqual(response.getcode(), 400)
        response_lower = response.read().decode("utf-8").lower()
    ...
    def tearDownClass(cls):
        subprocess.call(
            [
                "rm",
                "proxy_test_current_argument_values_artifact.json",
                "proxy_test_current_argument_values_artifact_from_proxy.json",
