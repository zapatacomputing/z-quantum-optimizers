...
def status():
    ...
    if request.method == "GET":
        ...
        if request_status not in ALLOWED_STATUSES:
            response = Response(
                "ERROR: Status: {} is invalid. Refer to documentation".format(
                    request_status
                )
            )
...
def cost_function_argument_values():
    ...
    if request.method == "GET":
        ...
        if OPTIMIZER_STATUS != "OPTIMIZING":
            response = Response(
                'ERROR: Cannot POST argument values while waiting for evaluation of previous argument values. Must wait until status is "OPTIMIZING".'
            )
        ...
        try:
            ...
            response = Response(
                "ERROR: Argument values: {} are invalid. Argument values must be in valid JSON format.".format(
                    request_body
                )
            )
            response.status_code = 400
            ...
            response = Response(
                "ERROR: Argument values: {} are invalid. Argument values must have a valid JSON type.".format(
                    request_body
                )
            )
            response.status_code = 400
            ...
            response = Response(
                "ERROR: Results: {} are invalid. Argument values must be a dict.".format(
                    request_body
                )
            )
            response.status_code = 400
...
def cost_function_results():
    ...
    if request.method == "GET":
        ...
        if requested_id != CURRENT_ID:
            response = Response(
                "ERROR: ID: {} is invalid. Requested ID must be current ID".format(
                    requested_id
                )
            )
            response.status_code = 400
        ...
        if OPTIMIZER_STATUS != "EVALUATING":
            response = Response(
                'ERROR: Cannot POST result while waiting for new argument values. Must wait until status is "EVALUATING".'
            )
            response.status_code = 409
            return response
        ...
        try:
            ...
            response = Response(
                "ERROR: Results: {} are invalid. Results must have a valid JSON format.".format(
                    request_body
                )
            )
            response.status_code = 400
            ...
            response = Response(
                "ERROR: Results: {} are invalid. Results must have a valid JSON type.".format(
                    request_body
                )
            )
            response.status_code = 400
            ...
            response = Response(
                "ERROR: Results: {} are invalid. Results must be a dict.".format(
                    request_body
                )
            )
            response.status_code = 400
        ...
        if "optimization-evaluation-id" not in results.keys():
            # bad request error
            response = Response(
                "ERROR: Results: {} is invalid. Results must contain optimization-evaluation-id field for verification.".format(
                    request_body
                )
            )
            response.status_code = 400
        ...
        if optimization_evaluation_id != CURRENT_ID:
            ...
            response = Response(
                "ERROR: ID: {} is invalid. Does not match ID: {} of argument values currently waiting for evaluation.".format(
                    optimization_evaluation_id, CURRENT_ID
                )
            )
            response.status_code = 409
            return response
