...
import numpy as np
import os
from .utils import load_optimization_results, save_optimization_results, validate_optimization_results
...
class TestUtils(unittest.TestCase):
    ...
    def test_optimization_result_io(self):
        ...
        opt_results['nit'] = 2
        opt_results['history'] = [
            {'value': 0.1, 'params': np.array([1., 2.])},
            {'value': -0.1, 'params': np.array([-1., 2.])}
        ]
        opt_results['opt_value'] = 5
        opt_results['opt_params'] = np.array([1., 2.])
        ...
        self.assertTrue(np.allclose(opt_results['opt_params'], loaded_result['opt_params']))
        ...
        for i in range(len(opt_results['history'])):
            self.assertAlmostEqual(opt_results['history'][i]['value'], loaded_result['history'][i]['value'])
            self.assertTrue(np.allclose(opt_results['history'][i]['params'], loaded_result['history'][i]['params']))
